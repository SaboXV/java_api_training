...
public class Maps {
    ...
    public void addBoat(int length, int x, int y, Orientation orientation) {
        ...
        while (length > 0) {
            map[x][y] = GameCell.BOAT;length--;coordinates.add(new Coordinates(x, y));
            switch (orientation) {
                case HORIZONTAL -> x++;
                case VERTICAL -> y++;
            }
    ...
    public Result hit(Coordinates coordinates) {
        if (getCell(coordinates) != GameCell.BOAT) return Result.MISS;
        var first = boats.stream().filter(s -> s.contains(coordinates)).findFirst();
        assert (first.isPresent());first.get().remove(coordinates);setCell(coordinates, GameCell.SUCCESSFUL_FIRE);
